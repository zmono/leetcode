...
func findWords(board [][]byte, words []string) (output []string) {
	...
	for i, row := range board {
		for j := range row {
			output = append(output, dfs(board, i, j, root)...)
		}
	}
	return
}

func dfs(board [][]byte, i, j int, p *Node) (output []string) {
	c := board[i][j]
	if c == '#' || p.next[c-'a'] == nil {
		return
	}
	p = p.next[c-'a']
	if p.word != "" {
		output = append(output, p.word)
		p.word = ""
	}
	board[i][j] = '#'
	if i > 0 {
		output = append(output, dfs(board, i-1, j, p)...)
	}
	if j > 0 {
		output = append(output, dfs(board, i, j-1, p)...)
	}
	if i < len(board)-1 {
		output = append(output, dfs(board, i+1, j, p)...)
	}
	...
	cases := []TestCase{
		...
		},
		{
			...
			[][]byte{
				...
				[]byte("vwxy"),
				[]byte("zzzz")},
			[]string{"aaaaaaaaaaaaaaaa", "aaaaaaaaaaaaaaab", "aaaaaaaaaaaaaaac", "aaaaaaaaaaaaaaad", "aaaaaaaaaaaaaaae", "aaaaaaaaaaaaaabc", "aaaaaaaaaaaaaabf", "aaaaaaaaaaaaaacb", "aaaaaaaaaaaaaacd", "aaaaaaaaaaaaaacg", "aaaaaaaaaaaaaadc", "aaaaaaaaaaaaaade", "aaaaaaaaaaaaaadh", "aaaaaaaaaaaaaaed", "aaaaaaaaaaaaaaei"},
		},
	}

	for _, c := range cases {
		fmt.Println(c)
		if output := findWords(c.board, c.words); !areEqual(output, c.output) {
			fmt.Printf("findWords(%v, %v) == %v != %v\n", c.board, c.words, output, c.output)
		}
	}
}
